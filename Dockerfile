...
FROM ghcr.io/openclaw/openclaw:2026.5.18@sha256:ac2c41d7122194d32258d1ec61b33079dbc498767ecadcd50849782ad5fcb057
...
# Configure gateway token auth when OPENCLAW_GATEWAY_TOKEN is set.
# Auto-generated by the Railway template [variables] block; visible/rotatable in Railway → Variables.
if [ -n "${OPENCLAW_GATEWAY_TOKEN:-}" ]; then
  if ! openclaw config set --batch-json \
    "[{\"path\":\"gateway.auth.mode\",\"value\":\"token\"},{\"path\":\"gateway.auth.token\",\"value\":\"$OPENCLAW_GATEWAY_TOKEN\"}]"; then
    echo "Warning: failed to persist gateway auth config"
  fi
else
  echo "Warning: OPENCLAW_GATEWAY_TOKEN not set; gateway running without auth"
fi
